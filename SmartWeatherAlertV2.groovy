  ...
  dynamicPage(name: 'mainPage', install: true, uninstall: true, submitOnChange: true) {
    ...
    def actions = location.helloHome?.getPhrases()*.label
        ...
        actions.sort()
            ...
            input "action", "enum", title: "Select an routine to execute", options: actions, multiple:true
            }
   		}   
  	}
}
      ...
      alertsFound.each { alertFound ->
        ...
        if( alert ) {
          if( alertFilter(alert.type) ) {
			...
			def msg = "Weather Alert! ${alert.description} from ${alert.date} until ${alert.expires}"	//displays the alert desc ("Tornado Warnin")
            //run routine
